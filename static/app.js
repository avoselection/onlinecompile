...
(() => {
  ...
  const state = {
    ...
    sidebarCollapsed: false, unreadChat: 0,
    // terminal
  ...
  const railParticipantsBtn= $("railParticipantsBtn");
  const railChatBtn        = $("railChatBtn");
  const railParticipantCount = $("railParticipantCount");
  const railUnread         = $("railUnread");
  const runBtn             = $("runBtn");
  ...
  function normChat(msg) {
    if (msg && typeof msg === "object") {
      ...
      return { from, fromId:msg.fromId||null, text:String(msg.text||""), color:msg.color||colorForParticipant(msg.fromId,from), ts:Number(msg.ts)||nowTs() };
    }
    const raw = String(msg??"");
    const sep = raw.indexOf(":");
    if (sep > 0) {
      const from = raw.slice(0,sep).trim()||"Система";
      return { from, fromId:null, text:raw.slice(sep+1).trimStart(), color:colorForParticipant(null,from), ts:nowTs() };
    }
    return { from:"Система", fromId:null, text:raw, color:"#ff92f9", ts:nowTs() };
  }
  function renderChat() {
    if (!chatBox) return;
    chatBox.innerHTML = "";
    ...
    if (!msgs.length) {
      ...
      msgs.forEach(m => {
        ...
        const time = document.createElement("span"); time.className = "chat-time"; time.textContent = formatClockTime(n.ts);
        head.append(auth, time);
  ...
  function appendChat(msg) {
    const norm = normChat(msg);
    state.chatMessages.push(norm);
    if (state.chatMessages.length > MAX_CHAT_MESSAGES)
      state.chatMessages.splice(0, state.chatMessages.length - MAX_CHAT_MESSAGES);
    // When the panel is collapsed, count unread messages from other people.
    if (state.sidebarCollapsed && norm.fromId && norm.fromId !== state.me?.id) {
      state.unreadChat++;
      updateRailCounts();
    }
    renderChat();
  }
  function clearChat() { state.chatMessages = []; state.unreadChat = 0; updateRailCounts(); renderChat(); }

  // ─── Collapsible sidebar ─────────────────────────────────────────
  function updateRailCounts() {
    if (railParticipantCount) railParticipantCount.textContent = String(state.participants.length);
    if (railUnread) {
      if (state.unreadChat > 0) {
        railUnread.hidden = false;
        railUnread.textContent = state.unreadChat > 99 ? "99+" : String(state.unreadChat);
      } else {
        railUnread.hidden = true;
  ...
  function setSidebarCollapsed(collapsed) {
    ...
    localStorage.setItem("livepy:sidebarCollapsed", state.sidebarCollapsed ? "1" : "0");
    const label = state.sidebarCollapsed ? "Развернуть панель" : "Свернуть панель";
    if (sidebarToggle) { sidebarToggle.setAttribute("aria-label", label); sidebarToggle.title = label; }
    if (sidebarRail) sidebarRail.setAttribute("aria-hidden", state.sidebarCollapsed ? "false" : "true");
    if (!state.sidebarCollapsed) state.unreadChat = 0; // opening the panel clears unread
    updateRailCounts();
    // The editor/terminal column resized — relayout once the CSS settles.
    requestAnimationFrame(() => { updateEditorLayout(); fitXterm(); renderRemoteCursors(); });
  }
  function sendChat() {
  ...
  function renderParticipants(list) {
    ...
    state.participants.forEach(p => {
      ...
      // Host-only: remove a student (with a 1-hour reconnect ban).
      if (isStudent && state.role === "host") {
        const kick = document.createElement("button");
        kick.type = "button";
        kick.className = "participant-kick";
        kick.title = "Удалить из комнаты (бан 1 час)";
        kick.setAttribute("aria-label", `Удалить ${p.name}`);
        kick.textContent = "✕";
        kick.addEventListener("click", () => {
          if (confirm(`Удалить «${p.name}» из комнаты? Повторное подключение будет недоступно в течение 1 часа.`)) {
            state.ws?.send(JSON.stringify({ type:"kick_student", target_id:p.id }));
          }
        });
  ...
  function handleMessage(msg) {
    switch (msg.type) {
      ...
      case "chat":
        appendChat({ from:msg.from, fromId:msg.from_id, text:msg.text, color:msg.color, ts:msg.ts });
        break;
      case "chat_throttled": {
        const now = Date.now();
      ...
      case "room_closed":
        ...
        break;
      case "kicked": {
        const reason = msg.message || "Преподаватель удалил вас из комнаты.";
        state.isRunning = false; updateButtons();
        state.manualClose = true; stopPingLoop(); stopAutosaveLoop(); stopHostReturnCountdown();
        setConnectionBadge("Удалены преподавателем","error");
        setReadOnly(true);
        try { state.ws?.close(); } catch {}
        showWorkspace(false); showEntryScreen(true);
  ...
  function bindCommonEvents() {
    ...
    railParticipantsBtn?.addEventListener("click", () => setSidebarCollapsed(false));
    railChatBtn?.addEventListener("click", () => setSidebarCollapsed(false));
  ...
  async function init() {
    ...
    bindCommonEvents();
    initKonamiCode();
    setSidebarCollapsed(localStorage.getItem("livepy:sidebarCollapsed") === "1");
